<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PayableManagement.aspx.cs" Inherits="KINS_SYSTEM.Pages.FinancialManagement.PayableManagement" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    ...
    <script>
        ...
        $(function () {
            ...
            //点击新增按钮
            $("#Pa_addNew").click(function () {
                payableAddnew();
            });

            //点击删除按钮
            $("#Pa_delete").click(function () {
                payableDelete();
            });
        ...
        //点击新增信息弹出窗口
        function payableAddnew() {
            showBlackScreen();
            $(".blackFrame").attr("src", "PayableAddnew.aspx");
        }

        //弹出删除窗口及进行确认
        function payableDelete() {
        ...
        function setPageData() {
            ...
            for (var i = (parseInt($("#pageNum").val()) - 1) * parseInt($(".tablePage select").val()) + 1 - starCom ; i < maxi ; i++) {
                ...
                tb += "<td><a class='pay'  name='" + i + "'>&nbsp;确认支款&nbsp;</a> <a class='linked edit' name='" + i + "'>编辑</a></td>";
                tb += "</tr>";
            }
            ...
            $(".edit").click(function () {
                ...
                showBlackScreen();
                $(".blackFrame").attr("src", "PayableEdit.aspx?data=" + safeUrlValue(data));
            });
            $(".pay").click(function () {
                var datas = JSON.stringify(pageData[$(this).attr("name")]);
                showBlackScreen();
                $(".blackFrame").attr("src", "PayableCheckout.aspx?data=" + safeUrlValue(datas));
            });
        }
...
<body>
    <form id="form1" runat="server">
        ...
        <div id="List">
            <span class="containerTitle">待出账信息<a id="Pa_addNew">&nbsp;+ 新增&nbsp;&nbsp;</a><a id="Pa_delete">&nbsp;- 删除&nbsp;&nbsp;</a><a id="export">&nbsp; 导出为Excel&nbsp;&nbsp;</a></span>
            <table class="comTable">
                <thead>
                    <tr>
                        <th style="width: 5%;">
                            <input id="checkAll" type="checkbox" /></th>
                        <th style="width: 10%">付款日期</th>
                        <th style="width: 10%">收款方</th>
                        <th style="width: 10%;">待支金额</th>
                        <th style="width: 15%">支款内容</th>
